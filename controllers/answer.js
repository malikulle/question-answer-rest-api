...
const addNewAnswerToRoute = asyncErrorWrapper(async (req, res, next) => {
...
const undoLikeAnswer = asyncErrorWrapper(async (req, res, next) => {
  ...
  if (!answer.likes.includes(req.user.id)) {
    return next(new CustomError("you can not like this answer"));
  }

  answer.likes.splice(answer.likes.indexOf(req.user.id));
